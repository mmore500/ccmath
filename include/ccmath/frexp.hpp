...
#ifndef CCMATH_FREXP_HPP
#define CCMATH_FREXP_HPP
...
inline constexpr Real frexp(Real arg, int* exp)
{
    if( std::is_constant_evaluated() )
    {
        return arg == Real(0)  ? detail::frexp_zero_impl(arg, exp) :
               ...
               ccmath::isinf(arg) ? detail::frexp_zero_impl(arg, exp) :
               ccmath::isnan(arg) ? detail::frexp_zero_impl(arg, exp) :
               ccmath::detail::frexp_impl(arg, exp);
    }
    else
    {
        using std::frexp;
...
{
    return ccmath::frexp(static_cast<double>(arg), exp);
}

inline constexpr float frexpf(float arg, int* exp)
{
    return ccmath::frexp(arg, exp);
}
